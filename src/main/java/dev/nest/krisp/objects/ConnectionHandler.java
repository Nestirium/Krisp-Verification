...
public class ConnectionHandler {
            ...
            CompletableFuture.runAsync(() -> {
                try (Connection connection = dataSource.getConnection()) {
                    String sql = String.format(
                            "INSERT INTO GUILDS(guild_id, prefix, enabled_verif, channelId_verif, reactionUnicode_verif, roleId_verif, messageId_verif, imageURL_verif, enabled_rules, channelId_rules, reactionUnicode_rules, roleId_rules, messageId_rules, imageURL_rules) VALUES ('%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s')",
                            guild.getId(),
                            data[0],
                            Utils.toInt(Boolean.parseBoolean(data[1])),
                            data[2],
                            data[3],
                            data[4],
                            data[5],
                            data[6],
                            Utils.toInt(Boolean.parseBoolean(data[7])),
                            data[8],
                            data[9],
                            data[10],
                            data[11],
    ...
    public void saveData(String guildId) {
        ...
        if (guild != null) {
            ...
            CompletableFuture.runAsync(() -> {
                try (Connection connection = dataSource.getConnection()) {
                    String sql = String.format("UPDATE GUILDS SET prefix='%s'" +
                                    "enabled_rules='%s'," +
                                    "channelId_rules='%s'," +
                                    "reactionUnicode_rules='%s'," +
                                    "roleId_rules='%s', " +
                                    "messageId_rules='%s'," +
                                    "imageURL_rules='%s'," +
                                    "enabled_verif='%s'," +
                                    "channelId_verif='%s'," +
                                    "reactionUnicode_verif='%s'," +
                                    "roleId_verif='%s'," +
                                    "messageId_verif='%s'," +
                                    "imageURL_verif='%s'," +
                                    "WHERE guild_id='%s'",
